...
class BioPubMedBert:
    ...
    def get_smilarity_score_for_medical_concept(self):
        ...
        similarities = [cosine_similarity([emb1], [emb2])[0][0] for emb1, emb2 in
                        zip(self.embeddings_term1, self.embeddings_term2)]
        ...
        self.medical_concepts.to_csv(r'medical_cocnept_simlarity_Score_pubmedBert.csv')
    ...
    def get_clinical_notes_embedding(self):
        ...
        self.similarity_score = self.compute_similarity(self.clinical_notes_df['embeddings'][0],
                                                        self.clinical_notes_df['embeddings'][1])
        logging.info(f"Similarity score between the 1st and 2nd  clinical notes:{self.similarity_score}")

        # Save the updated DataFrame with embeddings
        self.clinical_notes_df.to_csv(r'clinical_notes_with_biopubmed_bert_embeddings.csv', index=False)
    ...
    def word_embedding_visualisation(self,embedding):
        ...
        # word visualization
        embeddings_array=np.array(self.clinical_notes_df['embeddings'].to_list())
        ...
        for i, txt in enumerate(self.clinical_notes_df['notes'].head(14)):
            ...
            sampled_words = random.sample(words, min(len(words), 4))  # Sample up to 4 words from each sentence
...
def main():
    ...
    biopubmedbert = BioPubMedBert()
    biopubmedbert.get_smilarity_score_for_medical_concept()
    biopubmedbert.get_clinical_notes_embedding()
